...
pub fn TestPage() -> Element {
    ...
    rsx! {
        div { class: "space-y-6 pt-4",
            ...
            div {
                class: if dragging {
                    "border-2 border-dashed border-primary bg-primary/5 rounded-xl min-h-64 flex items-center justify-center transition-all duration-150"
                } else {
                    "border-2 border-dashed border-border rounded-xl min-h-64 flex items-center justify-center transition-all duration-150 hover:border-primary/40 hover:bg-accent/10"
                },

                if dragging {
                    div { class: "flex flex-col items-center gap-3 text-primary pointer-events-none select-none",
                        Upload { class: "size-12 animate-bounce" }
                        p { class: "text-lg font-medium", "Release to drop" }
                    }
