...
#[cfg(feature = "server")]
pub(crate) async fn run_mediainfo(bin: &str, path: &Path) -> Option<MediaInfoReport> {
    ...
    let tracks = parsed.pointer("/media/track")?.as_array()?;
